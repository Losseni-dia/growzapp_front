...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ invCount: 0, projCount: 0, totalInvesti: 0, totalCollecte: 0 });
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      const tInvesti = invs.reduce((sum: number, i: any) => sum + Number(i.montantInvesti || 0), 0);

      const tCollecte = projs.reduce((sum: number, p: any) => sum + Number(p.montantCollecte || 0), 0);
  ...
  const loadDividendesSummary = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get<ApiResponse<any[]>>("/api/dividendes/mes-dividendes");

      const dividendes = response.data || [];

      const payes = dividendes.filter((d: any) => d.statutDividende === "PAYE");

      const totalPercu = payes.reduce((sum: number, d: any) => sum + Number(d.montantTotal || 0), 0);
  ...
  return (
    ...
    <div className={styles.container}>
      ...
      {(user.kycStatus === "NON_SOUMIS" || user.kycStatus === "REJETE") && (
        ...
        <div className={`${styles.kycAlert} ${user.kycStatus === "REJETE" ? styles.kycError : ""}`}>
          ...
          <div className={styles.kycAlertText}>
            ...
            <strong>
              ...
              {user.kycStatus === "REJETE" ? t("kyc.status_rejected") : t("kyc.title")}
            ...
            <p>
              ...
              {user.kycStatus === "REJETE"
                ...
                : t("kyc.pending_action_hint") || "Veuillez vérifier votre identité pour débloquer toutes les fonctionnalités."}
          ...
          <Link to="/profile/kyc" className={styles.kycAlertBtn}>
            ...
            {user.kycStatus === "REJETE" ? t("kyc.retry_button") : t("kyc.btn_submit")}
      ...
      <section className={styles.profileSection}>
        ...
        <div className={styles.profileCard}>
          ...
          <div className={styles.info}>
            ...
            <p><FiPhone /> {user.contact || t("common.not_provided")}</p>
          ...
          <Link to="/wallet" className={styles.walletBadge}>
            ...
            <div className={styles.walletIcon}><FiDollarSign /></div>

            <div className={styles.walletLabel}>{t("dashboard.wallet.title")}</div>
            ...
            <span className={styles.walletLink}>{t("dashboard.wallet.view_detail")}</span>
      ...
      <section className={styles.statsSection}>
        ...
        <div className={styles.statsGrid}>
          ...
          <Link to="/mes-dividendes" className={styles.statCard}>
            ...
            <div className={styles.statIcon}><FiGift style={{ color: "#27ae60" }} /></div>
