...
import styles from "./CommandeFacturePage.module.css";

const API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:8080";

export default function CommandeFacturePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/commandes/${id}/facture`, {
      method: "GET",
      credentials: "include",
    })
      .then((res) => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <h1>
          <FiFileText /> {t("commande_facture.title", "Facture — Commande #{{id}}", { id })}
        </h1>
      </div>
      ...
      {loading ? (
        ...
        <div className={styles.viewer}>
          ...
          <a href={blobUrl} download={`facture-commande-${id}`} className={styles.btnDownload}>
            <FiDownload /> {t("commande_facture.btn_download", "Télécharger")}
          </a>
        </div>
      )}
    </div>
